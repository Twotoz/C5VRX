...
def cross(previous, current):
    return int(current - previous > 15 or current - previous < -16)


def lift(state6, current):
    previous = state6 & 31
    parity = ((state6 >> 5) ^ cross(previous, current)) & 1
    return current | (parity << 5)
...
def main():
    ...
    for p in range(32):
        for m in range(32):
            for c in range(32):
                ...
                u1 = lift(u0, m)
                u2 = lift(u1, c)
                assert signed6(u2 - u0) == wrap32(m - p) + wrap32(c - m)
                if winding(p, m, c) == 0:
                    ideal_vs_golden += pair_dac(p, m, c) != (GOLDEN[(p << 5) | c] & 63)
    ...
    print(f"Ideal P20/G2 differs from Golden on {ideal_vs_golden} no-winding triplets")
    print("Lifted-state adjacent50 triplets: 32768/32768 exact")
    ...
    raw_phase = [phase5(raw) for raw in range(256)]
    cross_rows = {
        tuple(cross(previous, current) for current in raw_phase)
        for previous in range(32)
    }
    assert len(cross_rows) == 32
    print("Distinct previous-phase cross rows over raw8: 32 (13 direct LUT bits)")
