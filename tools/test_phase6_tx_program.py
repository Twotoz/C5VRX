#!/usr/bin/env python3
"""Execute generated BitScrambler bundles against a raw IQ stream."""
...
from generate_phase6_tx import BSASM, dac, phase5_from_golden
...
def parse_program() -> tuple[list[list[str]], dict[str, int], list[int]]:
    ...
    for original in BSASM.read_text().splitlines():
        ...
        if not line.endswith(","):
            ...
            pending = []
    assert not pending and len(bundles) == 6 and len(lut) == 1024
...
def main() -> None:
    ...
    for index, word in enumerate(output):
        p, m, c = (phase[raw[1 + index * 2]],
                   phase[raw[2 + index * 2]],
                   phase[raw[3 + index * 2]])
        expected = dac(p, m, c)
        assert word == (expected | (expected << 8)), (index, word, expected, p, m, c)
    print(f"Phase6 TX program: {len(output)} raw-IQ output pairs byte-exact")
