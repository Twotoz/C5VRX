...
#define WIN_COPY      8192u                     /* private copy, indexed like the ring */
 ...
 * both indexed by absolute sample index & mask. */
...
static IRAM_ATTR bool find_edge(int64_t S_from, int64_t S_to, int thr, double *edge)
{
    ...
    const uint8_t *src = s_src;
    const uint8_t *ang = s_ang;
    const uint32_t mask = s_src_mask;
    const int thr0 = thr - EDGE_W * bias;
    uint32_t i = ((uint32_t)S_from - EDGE_W) & mask;
    uint8_t prev = ang[src[(i - 1u) & mask]];
    int8_t win[EDGE_W];
    int sum = 0;
...
static IRAM_ATTR void extract_pixels(int64_t S_act, uint8_t *dst, int32_t c0, int32_t m)
{
    ...
    if (i + (uint32_t)(GRAB_W * PIX_SAMPLES) <= s_src_mask) {
...
{
    static uint8_t done[GRAB_H], failed[GRAB_H];
    ...
    while (info->rows < GRAB_H && esp_timer_get_time() < t_end && s_locked) {
        ...
        for (int y = 0; y < GRAB_H && s_locked; ++y) {
            ...
            uint8_t *dst = img + (size_t)y * GRAB_W;
